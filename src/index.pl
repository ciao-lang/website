...
%   - @bf{assertions} (as a homogeneous framework that allows static and
%     dynamic verification to work cooperatively in a unified way), 
%   - @bf{multiparadigm} constructs (meta-programming, higher-order,
...
% TODO: Link to @ref{Bundles} (comprenhensive list of bundles)
% TODO: Link to playgrounds
...
% 
% % %  Ciao is a general-purpose programming language based on
% %  which supports:
% %  logic, constraint, functional, higher-order, and object-oriented
% %  programming styles. Its main design objectives are high expressive
% %  power, extensibility, safety, reliability, and efficient execution.
% %  
...
% %     architecture-independent and standalone executables, which run
